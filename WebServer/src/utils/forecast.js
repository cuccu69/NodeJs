...
const forecast = (latitude, longtitude, callback) => {
    ...
    return request({ url: url, json: true }, (error, { body }) => {
        if (error) {
            ...
            callback(undifined, `${body.current.weather_descriptions[0]}. Temperature is now ${body.current.temperature}oC and it feels like ${body.current.feelslike}oC out side`)
        }
    })
}
